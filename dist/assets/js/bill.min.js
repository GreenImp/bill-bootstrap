...
(function($, window, document){
	...
	$.extend({
		...
		getQueryString: function(name){
			...
			if(!cache[queryString]){
				...
				var queryVars = queryString.split('&'), arrayCheck = /^([^\[]+)((\[[^\]]*\])+)$/, arrayKeyCheck = /\[[^\]]*\]/g;
				...
				for(var i = 0; i < queryVars.length; i++){
					var variable = queryVars[i].split('='), key = decodeURIComponent(variable[0]), value = (typeof variable[1] == 'string') ? decodeURIComponent(variable[1].replace(/\+/g, ' ')) : undefined;
	...
	methods.browserNotice = function(options){
		...
		var supported = true, userAgent = navigator.userAgent;
		...
		if(!supported){
			...
			var animSpeed = 600, noticeBox = $('<div id="browserNotice" class="container">' + '<div class="row">' + '<div class="column eight">' + '<p>It looks like you\'re using an outdated web browser. We don\'t support old browsers, as they have limited features and security issues.</p>' + '<p>You should consider upgrading.</p>' + '</div>' + '<div class="column four">' + '<a href="http://browsehappy.com/" title="Upgrade your browser at Browse Happy" target="_blank"  class="button">Find out more</a>' + '</div>' + '</div>' + '</div>');

			$('body')// add the browser notice
					.prepend(noticeBox)// slide the whole page down, by the height of the notice (so page content doesn't get cut off)
	...
	methods.alert = function(type, msg, val, callback){
		if(msg){
			...
			var dialogueBox = $('<div id="alertPopup" class="alertBox float">' +										// the pop-up box
							'<div class="content"></div>' + '<div class="buttons"></div>' + '</div>'), overlay = ($('#alertOverlay').length > 0) ? $('#alertOverlay') : $('<div id="alertOverlay"></div>'),	// the overlay
			...
			switch(type){
				...
				case 'prompt':
					...
					content = '<p>' + msg + '</p>' + '<input type="text" name="value" value="' + (val || '') + '">';
			...
			dialogueBox.appendTo('body')					// add the dialogue box to the document body
			...
			$(window).on('resize', function(){
				dialogueBox.css({margin: 0, left: 0, top: 0})		// remove existing margin/positioning
						.css({								// add our own
							left: ($(window).width() / 2) - (dialogueBox.outerWidth() / 2),
							top: ($(window).height() / 2) - (dialogueBox.outerHeight() / 2)
						});
	...
	methods.accordion = function(userOptions){
		...
		return this.each(function(){
			...
			if(!data || !data.accordion){
				...
				var type = accordion.prop('tagName').toUpperCase(), titles = null, panes = null;
				...
				if(type == 'DL'){
					...
					var queryPrefixTitle = '', queryPrefixPane = '';
					if((type == 'UL') || (type == 'OL')){
					...
					titles.each(function(){
						...
						var pane = $(this).next(queryPrefixPane + '.pane, ' + queryPrefixPane + 'div:not(.title), ' + queryPrefixPane + 'article:not(.title), ' + queryPrefixPane + 'section:not(.title), ' + queryPrefixPane + 'aside:not(.title)').addClass('pane');
						panes = (panes == null) ? pane : panes.add(pane);
					});
				}
				 ...
				 */
				titles.addClass('title')			// ensure that the titles have the `title` class
			...
			if(!data || !data.tabs){
				...
				if(tabs.get(0).nodeName == 'DL'){
					...
					if(firstType == 'UL'){
						...
						tabs.children().each(function(i){
							...
							tabHtml += '<li>' + '<a href="#' + id + '">' + label + '</a>' + '</li>';
						});
				...
				if(tabBtns && (tabBtns.length > 0)){
					...
					// otherwise look for a button with class 'active'
							activeBtn = location.hash ? $(tabBtns.filter('[href="' + location.hash + '"]:first')[0] || tabBtns.filter('.active:first')[0]) : tabBtns.filter('.active:first');
					// if no tab buttons have a class of active, just go with the first tab

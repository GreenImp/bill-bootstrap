	...
	return this.each(function(){
		...
		if(!data || !data.tabs){
			...
			var tabBtns = tabs.children(':first'),
				firstType = tabBtns.get(0).nodeName,
				panes = null;
			...
			if(firstType == 'UL'){
			...
			}else if(firstType == 'DL'){
				// we are dealing with a dl element
				panes = tabBtns.find('dd');
				tabBtns = tabBtns.find('dt');
				...
				tabBtns.each(function(){
					...
					if(btn.children('a:first').length == 0){
						...
						// wrap the dt contents in the link
						btn.wrapInner('<a href="#' + id + '">');
